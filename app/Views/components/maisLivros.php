    ...
    <div class="container mx-auto px-4 max-w-7xl">
        <div class="text-center mb-8">
            ...
            <p class="text-sm text-gray-600 mt-1">
                Descubra mais opções para você
            </p>
        </div>

        <!-- Container do Slider -->
        <div class="relative">
            <button id="prevBtnMaisLivros"
            ...
            <div class="overflow-hidden mx-12 md:mx-16 slider-container-mobile">
                <div id="sliderTrackMaisLivros" class="flex transition-transform duration-500 ease-in-out draggable">
                    <?php 
                    // Duplicar livros para efeito infinito (3 cópias)
                    $livrosInfinitos = array_merge($maisLivros, $maisLivros, $maisLivros);
            ...
            <div class="flex justify-center mt-6 gap-2">
                <?php foreach ($maisLivros as $index => $livro): ?>
                    <button class="slider-dot-maisLivros w-2 h-2 rounded-full bg-gray-300 transition-all duration-300" 
...
document.addEventListener('DOMContentLoaded', function() {
    const sliderTrack = document.getElementById('sliderTrackMaisLivros');
    const prevBtn = document.getElementById('prevBtnMaisLivros');
    const nextBtn = document.getElementById('nextBtnMaisLivros');
    const dots = document.querySelectorAll('.slider-dot-maisLivros');
    
    const totalLivros = <?= count($maisLivros) ?>;
